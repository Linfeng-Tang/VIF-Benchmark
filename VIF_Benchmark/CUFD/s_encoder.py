...
class s_Encoder(object):
    ...
    def __init__(self, sco):
        ...
        with tf.variable_scope('s_encoder'):
            ...
            self.weight_vars.append(self._create_variables(96, 128, 3, scope='output_layer'))
    ...
    def encode(self, f1, f2):
        img = tf.concat([f1, f2], 3)
        ...
        for i in range(len(self.weight_vars)):
            ...
            if i == 0:
                out = conv2d(out, kernel, bias, dense=False, use_relu=True,
                                   Scope=self.scope + '/s_encoder/b' + str(i), BN=True, Reuse=tf.AUTO_REUSE)
                ...
                out = conv2d_1(out, kernel, bias, dense=False, use_relu=False,
                               Scope=self.scope + '/s_encoder/b' + str(i), BN=False, Reuse=tf.AUTO_REUSE)
            kernel_last = kernel[:]
            bias_last = bias[:]
...
def conv2d_1(x, kernel, bias, dense=False, use_relu=True, Scope=None, BN=True, Reuse=None):
    # padding image with reflection mode
