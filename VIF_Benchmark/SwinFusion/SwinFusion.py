...
def main(Method='SwinFusion', model_path='', ir_dir='', vi_dir='', save_dir='', is_RGB=True):  
    ...
    if os.path.exists(model_path):
        ...
        sys.exit()
    model = define_model(args, model_path=model_path)
    model.eval()
    ...
    for i, test_data in enumerate(test_bar):
        ...
        vi_path = test_data['B_path'][0] 
        util.imsave(output, save_name)        
        if is_RGB:
            util.img2RGB(save_name, vi_path)                
        test_bar.set_description('{} | {} | {:.4f} s'.format(Method, imgname, end-start))
        
def define_model(args, model_path=None):
